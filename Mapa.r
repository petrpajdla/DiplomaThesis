...
citation("ggmap")
# # ggmap - updated from github, need v7
# devtools::install_github("dkahle/ggmap")

# # READ DATA ==================================================================
# Lok <- read_csv("./Data/Db.Lok.csv")
...
Lok <- read_csv("./Data/Db.Lok.FromR.csv")

TabLok <- subset(Lok, select = c(Name, Type, n))
TabLok <- TabLok[order(Lok$Num),]
TabLokExp <- xtable(TabLok, 
                    caption = "Přehled lokalit, viz Obr. `\ref{ObrMapaLok}`, 
                    zkratky viz Tab. `\ref{TabKod}`")
print(TabLokExp, file = "../../Text/2Kap/TabLok.tex", 
...
LokMap <- get_map(location = Lok.bbox, 
                  maptype = "terrain-background", source = "stamen")
EurMap <- get_map(location = Eur.bbox, 
                  maptype = "terrain-background", source = "stamen")
...
FinEur <- ggmap(EurMap) +
  annotate("rect", xmin = 12, xmax = 19, ymin = 48, ymax = 51.2, 
           colour = "black", fill = NA, alpha = .3)
